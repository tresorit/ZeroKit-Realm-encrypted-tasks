...
final class TableViewCell<Item: Object>: UITableViewCell, UITextViewDelegate where Item: CellPresentable {
    ...
    var item: Item! {
        didSet {
            ...
            item.plainText { [weak self] (plainText, _) in
                if let plainText = plainText, let item = self?.item, item == newItem {
    ...
    func textViewDidEndEditing(_ textView: UITextView) {
        ...
        item.setPlainText(newText) { error in
            if let error = error {
                print("Failed to set text:", error)
                (UIApplication.shared.delegate as! AppDelegate).present(error: error as NSError)
            }
        }
